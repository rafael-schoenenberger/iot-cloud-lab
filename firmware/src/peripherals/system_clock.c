...
/* PLL config shared by both branches of SystemClock_Config()'s #if below.
 * Named PLL_M/_N/_Q, not PLLM/PLLN/PLLQ, to avoid colliding with the
 * identically-named RCC_PLLInitTypeDef fields set further down. */
#define PLL_M                25
#define PLL_N                336
#define PLL_Q                7
#define PLL_READY_TIMEOUT_MS 100
...
void SystemClock_Config(void)
{
    ...
    WRITE_REG(RCC->PLLCFGR, RCC_PLLSOURCE_HSE | PLL_M
    ...
    SET_BIT(RCC->CR, RCC_CR_PLLON);
    {
        ...
        {
            if((HAL_GetTick() - tickstart) > PLL_READY_TIMEOUT_MS)
            {
                Error_Handler();
            }
        }
