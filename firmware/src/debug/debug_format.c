...
/**
  * @brief  Formats one debug line as "[<uptime_ms>] <text>", clamping to
  *         `out_size` instead of overflowing it
  * @param  uptime_ms Timestamp to prefix the line with
  * @param  text NUL-terminated message text
...
{
    int len = snprintf(out, out_size, "[%lu] %s", (unsigned long)uptime_ms, text);
