...
/* Generous vs. millisecond-scale UART1/UART3 DMA transfers, for a stalled
 * transfer. UART1_TX_TIMEOUT_MS is shared by at_send() and at_send_cert();
 * UART3_TX_TIMEOUT_MS is used by at_send_retry()'s debug output. */
#define UART1_TX_TIMEOUT_MS 1000
#define UART3_TX_TIMEOUT_MS 1000

/* at_send_retry(): retry count, and the buffer for its failure debug line. */
